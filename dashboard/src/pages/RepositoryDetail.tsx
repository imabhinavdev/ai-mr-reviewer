  ...
  const issueChartData =
    issueSummary != null
      ? [
          {
            name: 'Clean PRs',
            value: issueSummary.reviewsWithNoIssues ?? 0,
            fill: PIE_COLORS[0],
          },
          {
            name: 'PRs with issues',
            value: issueSummary.reviewsWithIssues ?? 0,
            fill: PIE_COLORS[1],
          },
        ].filter((d) => d.value > 0)
      : []
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
          {decodedProvider === 'github' ? 'GitHub' : 'GitLab'} · Repository
          health
        </p>
      </div>
            ...
            <Card padding="md">
              ...
              <p className="mt-1 text-2xl font-semibold text-[var(--color-text)]">
                {overview?.avgReviewTimeSeconds != null
                  ? `${Math.round(overview.avgReviewTimeSeconds)}s`
                  : '—'}
              </p>
            </Card>
            <Card padding="md">
      ...
      <Card>
        <CardHeader
          ...
          subtitle="PRs with no issues vs PRs with AI feedback"
        />
        {issuesLoading && (
      ...
      >
        <Card padding="none">
          <div className="p-4 sm:p-5">
            <CardHeader
              title="Contributors"
              subtitle="Developers in this repository"
            />
            {contributors.length === 0 && (
              <EmptyState
                icon={<Users className="size-6" />}
                title="No contributors"
                description="No review activity by author yet."
              />
            )}
            {contributors.length > 0 && (
              <TableContainer>
                  ...
                  <tbody>
                    {contributors.map((c: RepoContributor, i: number) => (
                      <tr key={i} className={tableBodyRowClass}>
                        ...
                        >
                          {formatTimeAgo(c?.lastActivity ?? null)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <Card padding="none">
          <div className="p-4 sm:p-5">
            <CardHeader
              ...
              subtitle="Click a row to open review details"
            />
            {events.length === 0 && (
              <EmptyState
                icon={<ListChecks className="size-6" />}
                title="No recent reviews"
                description="Reviews will appear here when PRs are processed."
              />
            )}
            {events.length > 0 && (
              <TableContainer>
                  ...
                  <tbody>
                    {events.map((ev: ReviewEvent, index: number) => (
                      ...
                      >
                        ...
                        <td className={tableCellClass}>
                          {ev?.commentsPostedCount ?? 0}
                        </td>
                        <td
                          className={`${tableCellClass} text-[var(--color-text-secondary)]`}
                        >
                          {(ev as { aiProvider?: string })?.aiProvider ?? '—'}
                        </td>
                        <td
                          className={`${tableCellClass} text-[var(--color-text-secondary)]`}
                        >
                          {(ev as { durationSeconds?: number })
        ...
        >
          <Card>
            <CardHeader
              ...
              subtitle="How well the AI review system is working"
            />
      ...
      >
        <Card>
          <CardHeader
            ...
            subtitle={
              repoName
                ? `Jobs for this repo (refreshes every 10s)`
                : 'Repository-specific queue stats'
            }
          />
          {queueData != null && (
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <p className="mt-1 text-xl font-semibold text-[var(--color-text)]">
                  {queueData?.waiting ?? queueData?.pending ?? 0}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium text-[var(--color-text-secondary)]">
              ...
              <div>
                ...
                <p className="mt-1 text-xl font-semibold text-[var(--color-error)]">
                  {queueData?.failed ?? 0}
                </p>
              </div>
            </div>
          )}
          {queueData == null && repoName && (
            <p className="text-sm text-[var(--color-text-muted)]">
              Queue unavailable or no jobs for this repo.
            </p>
          )}
        </Card>
      </ErrorBoundary>
      ...
      <Card>
        ...
        {rulesData?.content != null ? (
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            ...
            found, or repo not accessible. Rules are read when each review runs.
          </p>
        )}
      </Card>
      ...
      <Card>
        <CardHeader
          ...
          subtitle="Per-repo configuration"
        />
        <p className="text-sm text-[var(--color-text-muted)]">
          Using global defaults. Per-repo settings (AI provider, review mode,
          ignore files, max diff size) can be added in a future update.
        </p>
      </Card>
        ...
        <Card>
          ...
          <div className="flex items-center gap-4">
            ...
            <ul className="list-disc list-inside text-sm text-[var(--color-text-secondary)] space-y-1">
              {(Array.isArray(healthScore?.reasons)
                ? healthScore.reasons
                : []
              ).map((r, i) => (
                <li key={i}>{r}</li>
