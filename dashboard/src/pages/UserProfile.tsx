...
export function UserProfile() {
  ...
  const insightsCategories =
    ...
    insights.categories != null &&
    typeof insights.categories === 'object'
      ? insights.categories
      : {}
  const categoryEntries = Object.entries(insightsCategories).sort(
  ...
  const reposChartData = reposList.map((r) => ({
    name:
      (r?.repoName?.length ?? 0) > 25
        ? (r?.repoName ?? '').slice(0, 22) + '...'
        : (r?.repoName ?? '—'),
    fullName: r?.repoName ?? '',
    count: r?.mrCount ?? 0,
  }))
  const recentReviews = Array.isArray(data?.recentReviews)
    ? data.recentReviews
    : []
  ...
  return (
    <div className="space-y-8">
        ...
        <Card>
          ...
          <dl className="grid gap-3 sm:grid-cols-2 text-sm">
            <div>
              ...
              <dd className="mt-0.5 text-[var(--color-text)] font-medium">
                {data?.username ?? '—'}
              </dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="mt-0.5 text-[var(--color-text)]">
                {data?.firstSeen
                  ? new Date(data.firstSeen).toLocaleDateString(undefined, {
                      month: 'short',
                      year: 'numeric',
                    })
                  : '—'}
            ...
            <div>
              ...
              <dd className="mt-0.5 text-[var(--color-text)]">
                {formatRelative(data?.lastActivity ?? null)}
              </dd>
            </div>
            <div>
              <dt className="font-medium text-[var(--color-text-secondary)]">
      ...
      <ErrorBoundary
        sectionLabel="Review statistics"
        message="This section failed to load."
        resetKeys={[data]}
      >
        <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            ...
            <Card>
              <div className="p-4">
                ...
                <p
                  className="mt-1 text-2xl font-semibold text-[var(--color-text)]"
                  title="Breakdown by severity coming soon"
                >
                  {data?.totalErrors ?? '—'}
                </p>
              </div>
      ...
      <Card>
        <CardHeader title="PRs created over time" subtitle="Last 30 days" />
      ...
      <Card>
        ...
        <ChartErrorBoundary chartName="Reviews per repository">
          {reposChartData.length === 0 ? (
            ...
            <div
              className="w-full"
              style={{ height: Math.max(200, reposChartData.length * 36) }}
            >
      ...
      {/* Repositories contributed */}
      <ErrorBoundary
        sectionLabel="Repositories contributed"
        message="This section failed to load."
        resetKeys={[data?.repos]}
      >
        <Card padding="none">
                  ...
                  <tbody>
                    {reposList.map((r, index) => (
                      ...
                      >
                        ...
                        >
                          {r?.lastContribution
                            ? formatRelative(r.lastContribution)
                            : '—'}
                        </td>
                      </tr>
                    ))}
      ...
      <ErrorBoundary
        sectionLabel="Recent review activity"
        message="This section failed to load."
        resetKeys={[data?.recentReviews]}
      >
        <Card padding="none">
          <div className="p-4 sm:p-5">
            <CardHeader
      ...
      {/* Code quality insights */}
      <ErrorBoundary
        sectionLabel="Code quality summary"
        message="This section failed to load."
        resetKeys={[data]}
      >
        <Card>
          ...
          <div className="space-y-3 text-sm">
            <p className="text-[var(--color-text-secondary)]">
              <strong className="text-[var(--color-text)]">
                Total issues found by AI:
              </strong>{' '}
              {data?.totalComments ?? '—'}
            </p>
            {(data?.totalErrors != null || data?.totalWarnings != null) && (
              <p className="text-[var(--color-text-secondary)]">
                Errors: {data?.totalErrors ?? 0} · Warnings:{' '}
                {data?.totalWarnings ?? 0}
              </p>
            )}
            <p className="text-[var(--color-text-secondary)] mt-2">
              Issue categories (e.g. error handling, security, unused variables)
              require storing detailed review feedback and will be available in
              a future release.
            </p>
          </div>
        </Card>
